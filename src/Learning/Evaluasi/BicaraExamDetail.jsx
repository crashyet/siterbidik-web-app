...
const BicaraExamDetail = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const response = await assignmentAPI.getSubmissions('bicara', exam.id)
  ...
  const fetchMySubmission = async () => {
    try {
      const response = await assignmentAPI.getMySubmission('bicara', exam.id)
      if (response.data) {
        setMySubmission(response.data)
        setAssignmentStatus('Selesai')
        setIsSubmitted(true)
        // If there's a file, we can optionally show it or just show as submitted
        ...
        {activeTab === 'petunjuk' && (
          <>
            ...
            <div className="flex items-center justify-between gap-4 mb-2">
              <h1 className="text-base font-bold text-black">{exam.title}</h1>
        ...
        <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-6 animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden relative shadow-2xl">
            ...
            <div className="bg-purple-main p-6 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <img src={currentSubmission.avatar} alt="" className="w-10 h-10 rounded-full border-2 border-white/30" />
                <div>
            ...
            <div className="p-8 flex flex-col items-center">
              <div className="w-20 h-20 bg-purple-50 rounded-full flex items-center justify-center mb-6">
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none">
                  <path d="M12 3V12M12 12V21M12 12H21M12 12H3" stroke="#9747FF" strokeWidth="2" strokeLinecap="round"/>
                  <circle cx="12" cy="12" r="9" stroke="#9747FF" strokeWidth="2"/>
                  <path d="M9 12L12 9L15 12" stroke="#9747FF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              
              <h4 className="text-black font-bold text-sm mb-6">Rekaman Jawaban Siswa</h4>
              
              {/* HTML5 Audio Player */}
              <audio 
                controls 
                autoPlay 
                className="w-full h-10 mb-4 accent-purple-main"
              ...
              <p className="text-gray-400 text-[10px] text-center italic">
                Klik play untuk mendengarkan jawaban siswa
              </p>
            </div>
      ...
      {isGradingModalOpen && currentSubmission && (
        <div className="fixed inset-0 bg-black/80 z-[70] flex items-center justify-center p-6 animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden relative shadow-2xl">
            {/* Modal Header */}
            <div className="bg-purple-main p-6 text-white flex items-center justify-between">
              <h3 className="font-bold text-base">Penilaian Tugas</h3>
              <button 
